...
export const useBuiltStations = (stations) => {
	...
	const clearBuiltStations = () => {
		// Buscar el id del stash en las estaciones
		const stashId = stations.find(
			(s) => s.normalizedName === "stash" || s.name.toLowerCase() === "stash"
		)?.id;

		setBuiltStations((prev) => {
			if (stashId && prev[stashId]) {
				// Solo conservar el stash
				return { [stashId]: prev[stashId] };
			}
			return {};
		});

		// Actualizar localStorage acorde
		const current = JSON.parse(localStorage.getItem("builtStations") || "{}");
		if (stashId && current[stashId]) {
			localStorage.setItem("builtStations", JSON.stringify({ [stashId]: current[stashId] }));
		} else {
			localStorage.removeItem("builtStations");
		}
	};
	...
	return { builtStations, handleStationLevelChange, clearBuiltStations };
};
